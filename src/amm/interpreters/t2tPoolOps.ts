...
import {make as makeBoxCandidate} from "../../wallet/entities/ergoBoxCandidate";
import {make as makeTxCandidate} from "../../wallet/entities/ergoTxCandidate";
...
export class T2tPoolOps implements PoolOps {
    ...
    async setup(params: PoolSetupParams, ctx: TransactionContext): Promise<ErgoTx[]> {
        ...
        if (pairIn.length == 2) {
            ...
            let txc0 = makeTxCandidate(ctx.inputs, [proxyOut], height, ctx.feeNErgs, ctx.changeAddress)
            let tx0 = await this.prover.sign(txc0)

            let lpP2Pk = ergoTreeFromAddress(ctx.changeAddress)
            let lpShares = {id: tokenIdLP, amount: params.outputShare}
            let lpOut = makeBoxCandidate(MinBoxAmountNErgs, lpP2Pk, height, [lpShares])
            ...
            let poolLP = {id: tokenIdLP, amount: poolAmountLP}
            ...
            let poolOut = makeBoxCandidate(poolValueNErgs, poolScript, height, poolTokens, poolRegis, newTokenNFT)
            let txc1Inputs = BoxSelection.safe(poolBootBox)
            let txc1 = makeTxCandidate(txc1Inputs, [poolOut, lpOut], height, ctx.feeNErgs, ctx.changeAddress)
    ...
    deposit(params: DepositParams, ctx: TransactionContext): Promise<ErgoTx> {
        ...
        if (pairIn.length == 2) {
            let out = makeBoxCandidate(outputGranted.nErgs, proxyScript, ctx.network.height, pairIn)
            let txc = makeTxCandidate(ctx.inputs, [out], ctx.network.height, ctx.feeNErgs, ctx.changeAddress)
    ...
    redeem(params: RedeemParams, ctx: TransactionContext): Promise<ErgoTx> {
        ...
        if (tokensIn.length == 1) {
            let out = makeBoxCandidate(outputGranted.nErgs, proxyScript, ctx.network.height, tokensIn)
            let txc = makeTxCandidate(ctx.inputs, [out], ctx.network.height, ctx.feeNErgs, ctx.changeAddress)
            return this.prover.sign(txc)
        } else {
