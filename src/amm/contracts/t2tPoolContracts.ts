...
export class T2tPoolContracts {
  static poolBoot(emissionLP: bigint): ErgoTree {
    const tree = RustModule.SigmaRust.ErgoTree.from_base16_bytes(templates.T2tPool);
    tree.set_constant(
      7,
  ...
  static pool(emissionLP: bigint): ErgoTree {
    const tree = RustModule.SigmaRust.ErgoTree.from_base16_bytes(templates.T2tPoolBoot);
    tree.set_constant(
      6,
      RustModule.SigmaRust.Constant.from_i64(RustModule.SigmaRust.I64.from_str(emissionLP.toString()))
    );
  ...
  static deposit(
    ...
    const tree = RustModule.SigmaRust.ErgoTree.from_base16_bytes(templates.T2tDeposit);
  ...
  static redeem(
    ...
    const tree = RustModule.SigmaRust.ErgoTree.from_base16_bytes(templates.T2tRedeem);
    tree.set_constant(0, RustModule.SigmaRust.Constant.decode_from_base16(pk));
    tree.set_constant(
      7,
      RustModule.SigmaRust.Constant.from_i64(RustModule.SigmaRust.I64.from_str(emissionLP.toString()))
  ...
  static swap(
    ...
    const tree = RustModule.SigmaRust.ErgoTree.from_base16_bytes(templates.T2tSwap);
    ...
    tree.set_constant(
      9,
      RustModule.SigmaRust.Constant.from_byte_array(fromHex(poolScriptHash))
    );
    tree.set_constant(
      11,
