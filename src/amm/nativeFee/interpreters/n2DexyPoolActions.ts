...
import * as N2Dexy from "../contracts/n2dexyPoolContracts"

export class N2DexyPoolActions implements PoolActions<TxRequest, NativeExFeeType> {
  ...
  redeem(params: RedeemParams<NativeExFeeType>, ctx: TransactionContext): Promise<TxRequest> {
    // TODO: complete dex actions for dexy
    const proxyScript = N2Dexy.redeem(params.poolId, params.pk, params.exFee, ctx.feeNErgs)
    const outputGranted = ctx.inputs.totalOutputWithoutChange
  ...
  async swap(params: SwapParams<NativeExFeeType>, ctx: TransactionContext): Promise<TxRequest> {
    const out = await (isNative(params.baseInput.asset)
      ? N2DexyPoolActions.mkSwapSell(params, ctx)
      : N2DexyPoolActions.mkSwapBuy(params, ctx))
    const uiRewardOut: ErgoBoxCandidate[] = this.mkUiReward(ctx.network.height, params.uiFee)
    return {
      inputs: ctx.inputs,
      dataInputs: [],
  ...
  private static async mkSwapSell(
    ...
    const proxyScript = N2Dexy.swapSell(
      params.poolId,
  ...
  private static async mkSwapBuy(
    ...
    const proxyScript = N2Dexy.swapBuy(
      params.poolId,
      params.poolFeeNum,
      params.minQuoteOutput,
      params.exFeePerToken,
