...
export class AmmPool {
    ...
    depositAmount(input: AssetAmount): AssetAmount {
        if (input.asset === this.assetX)
            ...
            return this.x.withAmount(input.amount * this.priceY.numerator / this.priceY.denominator)
    }
    ...
    inputAmount(output: AssetAmount, maxSlippage?: number): AssetAmount | undefined {
        ...
        if (output.asset === this.assetX && minimalOutput > 0) {
            return this.y.withAmount(this.y.amount * output.amount * this.feeDenom /
                ((this.x.amount + this.x.amount * slippage / 100n - output.amount) * this.feeNum) + 1n)
        } else if (output.asset === this.assetY && minimalOutput > 0) {
            return this.x.withAmount(this.x.amount * output.amount * this.feeDenom /
                ((this.y.amount + this.y.amount * slippage / 100n - output.amount) * this.feeNum) + 1n)
        } else {
            return undefined
        }
    }
    ...
    outputAmount(input: AssetAmount, maxSlippage?: number): AssetAmount {
        let slippage = BigInt(maxSlippage || 0)
        if (input.asset === this.assetX) {
            return this.y.withAmount(this.y.amount * input.amount * this.feeNum /
                (this.x.amount + this.x.amount * slippage / 100n) * this.feeDenom + input.amount * this.feeNum)
        } else {
            return this.x.withAmount(this.x.amount * input.amount * this.feeNum /
                (this.y.amount + this.y.amount * slippage / 100n) * this.feeDenom + input.amount * this.feeNum)
        }
    }
}
