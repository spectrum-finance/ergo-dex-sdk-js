...
import { AssetAmount, AssetInfo } from '../../wallet';
...
export class AmmPool {
  ...
  private feeDenom = 1000n;
  private feeNum = BigInt(this.poolFeeNum);
  ...
  depositAmount(input: AssetAmount): AssetAmount {
    const price = input.asset === this.assetX ? this.priceX : this.priceY;
  ...
  inputAmount(
    ...
    const slippage = BigInt(maxSlippage || 0);
    ...
    if (output.asset === this.assetX && minimalOutput > 0) {
      return this.y
        ...
        .div(
          this.x
            .add(this.x.mul(slippage).div(100n))
            .sub(output)
            .mul(this.feeNum)
        )
        .add(1n);
  ...
  outputAmount(input: AssetAmount, maxSlippage?: number): AssetAmount {
    ...
    if (input.asset === this.assetX) {
      return this.y
        .mul(input.mul(this.feeNum))
        .div(
          this.x
            .add(this.x.mul(slippage).div(100n))
            .mul(this.feeDenom)
            .add(input.mul(this.feeNum))
        );
    } else {
      return this.x
        .mul(input.mul(this.feeNum))
        .div(
          this.y
            .add(this.y.mul(slippage).div(100n))
            .mul(this.feeDenom)
            .add(input.mul(this.feeNum))
        );
    }
